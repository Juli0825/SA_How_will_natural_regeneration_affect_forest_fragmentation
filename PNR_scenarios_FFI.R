...
output_dir      <- "R:/Chapter_3_fragmentation/2026_NEE_R2/FFI_results/hh_qiebai"
...
writeRaster(r_scenario_ffi,
            file.path(output_dir,
                      paste0(scenario_name, "_hh_b_FFI_raster.tif")),
            overwrite = TRUE)
writeRaster(r_delta_cont,
            file.path(output_dir,
                      paste0("delta_FFI_", scenario_name,
                             "_hh_b_continuous.tif")),
            overwrite = TRUE)
writeRaster(r_delta_class,
            file.path(output_dir,
                      paste0("delta_FFI_", scenario_name,
                             "_hh_b_classified.tif")),
            overwrite = TRUE)
writeRaster(r_delta_ed,
            file.path(output_dir,
                      paste0("delta_ED_", scenario_name, "_hh_b.tif")),
            overwrite = TRUE)
writeRaster(r_delta_pd,
            file.path(output_dir,
                      paste0("delta_PD_", scenario_name, "_hh_b.tif")),
            overwrite = TRUE)
writeRaster(r_delta_mpa,
            file.path(output_dir,
                      paste0("delta_MPA_", scenario_name, "_hh_b.tif")),
            overwrite = TRUE)
...
boundaries_path <- "R:/Chapter_3_fragmentation/2026_NEE_R2/FFI_results/global_boundaries_10m_current_forest.csv"
output_dir      <- "R:/Chapter_3_fragmentation/2026_NEE_R2/FFI_results/ap_qiebai"
mollweide_crs   <- "+proj=moll +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m +no_defs"
round_digits    <- 10
